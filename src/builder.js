...
function getPageTemplate(prefix, page, lang, namespaces) {
  ...
  return `// @ts-nocheck
import I18nProvider from '${prefix}/src/I18nProvider'
...
function buildPageLocale({ prefix, pagePath, namespaces, lang, path }) {
  ...
  // allLanguages.forEach(async (lang) => {
    const dirs = finalPath.replace(`/${filename}`, `/${filenameWithoutExtension}/${lang}`)
    const indexFinal = dirs.concat('/index.js');
    fs.mkdirSync(dirs, { recursive: true })
    fs.writeFileSync(indexFinal.replace(/(\.tsx|\.ts)$/, '.js'), template)
  // })
}
...
import C from './index/${defaultLanguage}/index'
...
export * from './index/${defaultLanguage}/index'
`
}
...
export default function CatchAll() {
  const router = useRouter()
  if (Array.isArray(router.query.path) && typeof window !== 'undefined') {
    if (router.query.path[router.query.path.length - 1] === '${defaultLanguage}') {
      return <Error statusCode="404" />
    }
    router.replace(\`/\${router.query.path.join('/')}/${defaultLanguage}\`)
  }
  return null
}
`
